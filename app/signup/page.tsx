...
function SignupContent() {
  ...
  const tierDetails: Record<PlanKey, { mins: string; priceMonthly: number; color: string }> = {
    starter: { mins: '300', priceMonthly: 149, color: 'text-emerald-400' },
    professional: { mins: '1,200', priceMonthly: 499, color: 'text-emerald-400' },
    growth: { mins: '3,000', priceMonthly: 999, color: 'text-emerald-400' },
    enterprise: { mins: '7,000+', priceMonthly: 1999, color: 'text-emerald-400' },
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-emerald-500/30 flex items-center justify-center p-6 font-sans relative overflow-hidden">
      {/* Cinematic Background */}
      <div className="absolute inset-0 z-0 opacity-30 pointer-events-none">
        <div className="absolute inset-0 bg-gradient-to-b from-emerald-900/20 via-black to-black" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-emerald-500/10 via-transparent to-transparent blur-3xl" />
      </div>

      <div className="max-w-md w-full relative z-10">
        <div className="text-center mb-10">
          <Link href="/" className="inline-block mb-8 group">
             <h1 className="text-xl font-black italic uppercase tracking-tighter text-white group-hover:text-emerald-400 transition-colors">FrontDesk // Control Node</h1>
          </Link>

          <h2 className="text-5xl font-black italic uppercase tracking-tighter mb-4">Initialize <span className="text-emerald-500">Fleet</span></h2>

          <div className="flex items-center justify-center gap-3 font-mono text-[10px] uppercase tracking-[0.3em] mb-10">
            <span className={`${currentTier.color} font-black`}>{selectedTier} tier</span>
            <span className="text-white/10">|</span>
            <span className="text-slate-500">{currentTier.mins} mins included</span>
          </div>

          <div className="inline-flex items-center gap-1.5 bg-white/5 border border-white/10 rounded-2xl p-1.5 backdrop-blur-xl">
            <button
              type="button"
              ...
              className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${
                billingCycle === 'monthly' ? 'bg-emerald-500 text-black shadow-[0_0_20px_rgba(16,185,129,0.4)]' : 'text-slate-500 hover:text-white'
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 ${
                billingCycle === 'annual' ? 'bg-emerald-500 text-black shadow-[0_0_20px_rgba(16,185,129,0.4)]' : 'text-slate-500 hover:text-white'
              }`}
            >
              Annual
              <span className="text-[8px] bg-black/20 text-black px-2 py-0.5 rounded-full font-black">-20%</span>
            </button>
          </div>
        </div>

        <div className="bg-white/5 border border-white/10 rounded-[3rem] p-10 shadow-2xl backdrop-blur-2xl relative overflow-hidden">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {errors.submit && (
              <p className="text-[10px] text-red-500 uppercase font-black text-center bg-red-500/5 py-3 rounded-xl border border-red-500/20">{errors.submit}</p>
            )}

            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full bg-emerald-500 text-black py-6 rounded-2xl font-black uppercase tracking-[0.2em] text-xs hover:bg-emerald-400 transition-all shadow-[0_15px_30px_rgba(16,185,129,0.25)] active:scale-[0.98] flex items-center justify-center gap-3"
            >
              {isSubmitting ? (
          ...
          <div className="mt-12 pt-8 border-t border-white/10 flex justify-between items-center text-[9px] font-black text-slate-500 uppercase tracking-widest">
        ...
        <p className="mt-12 text-center text-[10px] text-slate-500 uppercase tracking-[0.2em] font-bold">
          Active node in possession?{' '}
          <Link href="/login" className="text-white hover:text-emerald-400 underline transition-colors underline-offset-4">
            Authorize Entry
          </Link>
        </p>
      </div>
  ...
  return (
    <div className="space-y-3">
      ...
      <input
        ...
        className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-5 text-sm font-bold text-white placeholder-white/10 focus:border-emerald-500 transition-all outline-none backdrop-blur-md"
      />
    </div>
  );
}
...
export default function SignupPage() {
  return (
    <Suspense fallback={<div className="bg-black min-h-screen flex items-center justify-center text-emerald-500 font-black uppercase tracking-[0.3em]">Syncing Node...</div>}>
      <SignupContent />
    </Suspense>
  );
}
